...
async def seed_users_and_permissions(session: AsyncSession):
    ...
    permissions = [
        ...
        # 角色管理
        Permission(code="role:view", name="查看角色", resource="role", action="view", description="查看角色列表和详情"),
        Permission(code="role:create", name="创建角色", resource="role", action="create", description="创建新角色"),
        Permission(code="role:edit", name="编辑角色", resource="role", action="edit", description="编辑角色信息"),
        Permission(code="role:delete", name="删除角色", resource="role", action="delete", description="删除角色"),
        Permission(code="role:assign-permission", name="分配角色权限", resource="role", action="assign-permission", description="为角色分配权限并为用户分配角色"),
