...
from app.schemas.budget import BudgetAnalysisResponse, BudgetBatchCreate
...
async def batch_save_budgets(
    ...
    await budget_service.batch_save_budgets(
        db, data.store_id, data.year, data.month, data.items, current_user
    )
...
@router.get(
    "/analysis",
    response_model=Response[BudgetAnalysisResponse],
    summary="获取预算分析报表",
)
