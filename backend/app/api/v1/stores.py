...
from app.schemas.common import PageData, Response, success
from app.schemas.store import StoreCreate, StoreInDB, StoreListQuery, StoreUpdate
...
async def get_all_stores(
    ...
    return success(data=[StoreInDB.model_validate(store) for store in stores])
    ...
    if query_params.order_by:
        if query_params.order_by == "name":
            query = query.order_by(
                Store.name.desc() if query_params.desc else Store.name.asc()
            )
        elif query_params.order_by == "created_at":
            query = query.order_by(
                Store.created_at.desc() if query_params.desc else Store.created_at.asc()
            )
    else:
...
async def create_store(
    ...
    """创建门店"""
    # 检查名称是否已存在
    existing = await db.execute(select(Store).where(Store.name == store_data.name))
    if existing.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="门店名称已存在"
        )
    ...
    return success(data=StoreInDB.model_validate(store), message="门店创建成功")
...
async def update_store(
    ...
    return success(data=StoreInDB.model_validate(store), message="门店更新成功")
