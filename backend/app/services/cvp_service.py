...
async def update_cost_behavior(
    db: AsyncSession, expense_type_id: int, cost_behavior: str
):
    """
    更新费用类型的成本习性
    """
...
async def calculate_cvp(
    ...
    ).where(KpiDailyStore.biz_date >= start_date, KpiDailyStore.biz_date <= end_date)
    ...
    expense_query = select(
        ExpenseRecord.expense_type_id, func.sum(ExpenseRecord.amount)
    ).where(
    ...
    break_even_point = (
        (total_fixed_cost / (contribution_margin_rate / 100))
        if contribution_margin_rate > 0
        else 0
    )
    break_even_sales_ratio = (
        (break_even_point / total_revenue * 100) if total_revenue > 0 else 0
    )

    # 安全边际
    safety_margin = total_revenue - break_even_point
    safety_margin_rate = (
        (safety_margin / total_revenue * 100) if total_revenue > 0 else 0
    )
    ...
    # 经营杠杆系数
    operating_leverage = (
        (contribution_margin / operating_profit) if operating_profit != 0 else 0
    )
...
async def simulate_cvp(
    ...
    """
    # 获取原始数据
    original = await calculate_cvp(
        db, store_id, start_date, end_date, accessible_store_ids
    )
    ...
    new_contribution_margin_rate = (
        (new_contribution_margin / original.total_revenue * 100)
        if original.total_revenue > 0
        else 0
    )

    # 新的盈亏平衡点
    new_bep = (
        (new_fixed_cost / (new_contribution_margin_rate / 100))
        if new_contribution_margin_rate > 0
        else 0
    )

    bep_change = new_bep - original.break_even_point
    bep_change_rate = (
        (bep_change / original.break_even_point * 100)
        if original.break_even_point > 0
        else 0
    )
