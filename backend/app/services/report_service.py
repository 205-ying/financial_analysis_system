...
async def get_daily_summary(
    db: AsyncSession, filters: ReportQuery, current_user: User
    ...
    # 数据权限过滤：限制可访问的门店
    accessible_store_ids = await filter_stores_by_access(
        db, current_user, filters.store_id
    )
    ...
    if filters.group_by == "day":
        query = query.group_by(
            KpiDailyStore.biz_date, KpiDailyStore.store_id, Store.name
    ...
    expense_dict = {
        (row.biz_date, row.store_id): row.expense_total for row in expense_result.all()
    }
    ...
    order_dict = {
        (row.biz_date, row.store_id): row.order_count for row in order_result.all()
    }
    ...
    for row in rows:
        ...
        if row.revenue and row.revenue > 0:
            gross_profit_rate = (row.gross_profit / row.revenue * 100).quantize(
                Decimal("0.01")
            )
            operating_profit_rate = (row.operating_profit / row.revenue * 100).quantize(
                Decimal("0.01")
            )
...
async def get_store_performance(
    ...
    query = query.group_by(KpiDailyStore.store_id, Store.name).order_by(
        func.sum(KpiDailyStore.revenue).desc()
    )
    ...
    for idx, row in enumerate(rows, start=1):
        order_count, avg_order_amount = order_dict.get(
            row.store_id, (0, Decimal("0.00"))
        )
    ...
    sorted_by_profit = sorted(
        result_list, key=lambda x: x.operating_profit, reverse=True
    )
...
async def get_expense_breakdown(
    ...
    # 计算总费用（用于占比计算）
    total_query = select(func.sum(ExpenseRecord.amount).label("grand_total")).where(
        and_(
            ExpenseRecord.biz_date >= filters.start_date,
            ExpenseRecord.biz_date <= filters.end_date,
        )
    ...
    for row in rows:
        ...
        percentage = None
        if grand_total > 0:
            percentage = (row.total_amount / grand_total * 100).quantize(
                Decimal("0.01")
            )
...
async def export_report_excel(
    ...
    for row_num, data in enumerate(daily_data, start=2):
        ...
        ws1.cell(
            row=row_num,
            column=15,
            value=float(data.gross_profit_rate) if data.gross_profit_rate else None,
        )
        ws1.cell(
            row=row_num,
            column=16,
    ...
    for row_num, data in enumerate(expense_data, start=2):
        ...
        ws3.cell(
            row=row_num,
            column=10,
            value=float(data.percentage) if data.percentage else None,
        )
