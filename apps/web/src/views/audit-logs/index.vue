    ...
    <el-card shadow="never">
      <template #header>
        <div class="interactive-table-header">
          <div>
            <span class="interactive-table-title">审计日志</span>
...
import { useAsyncTask, useInteractiveTable } from '@/composables'
import { 
  getAuditLogs, 
  getAuditActions, 
  getResourceTypes,
...
const normalizeAllOption = (value: string | undefined) => value === allOptionValue ? undefined : value
...
const {
  ...
  refresh
} = useInteractiveTable<AuditLog, AuditLogQuery>(queryForm, getAuditLogs, {
  dateRange,
  errorMessage: '审计日志查询失败'
})
 ...
 */
const handleQuery = () => {
  queryForm.action = normalizeAllOption(queryForm.action)
  queryForm.resource_type = normalizeAllOption(queryForm.resource_type)
  queryForm.status = normalizeAllOption(queryForm.status)
 ...
 * 加载操作类型和资源类型
 */
const { execute: loadOptions } = useAsyncTask(async () => {
  try {
