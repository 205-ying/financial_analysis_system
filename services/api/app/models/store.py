...
class Store(Base, IDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="门店编码"
    )
    ...
    address: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="门店地址"
    )
    ...
    area_sqm: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 2), nullable=True, comment="营业面积（平方米）"
    )

    # 状态信息
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False, comment="是否营业"
    )
...
class ProductCategory(Base, IDMixin, TimestampMixin):
    ...
    # 基本信息
    code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="分类编码"
    )
...
class Product(Base, IDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    sku_code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="SKU编码"
    )
    ...
    unit: Mapped[str] = mapped_column(
        String(20), default="份", nullable=False, comment="单位"
    )
    ...
    image_url: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="产品图片URL"
    )

    # 状态信息
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, nullable=False, comment="是否在售"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<Product(id={self.id}, sku_code='{self.sku_code}', name='{self.name}')>"
        )
