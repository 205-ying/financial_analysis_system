...
class AuditLog(Base, IDMixin, TimestampMixin):
    ...
    resource: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="资源名称或操作对象"
    )
    resource_type: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="资源类型：expense/kpi/order/store/user等"
    )
    ...
    method: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="HTTP方法：GET/POST/PUT/DELETE等"
    )
    ...
    detail: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="操作详情（JSON格式），不包含敏感信息如密码"
    )

    # 请求信息
    ip_address: Mapped[str | None] = mapped_column(
        String(45), nullable=True, comment="客户端IP地址（支持IPv6）"
    )
    user_agent: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="客户端User-Agent"
    )
    ...
    error_message: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="错误信息（仅在失败时记录）"
    )

    # 关联关系（使用 viewonly=True 避免关系冲突警告）
    user: Mapped[User | None] = relationship(
        "User", foreign_keys=[user_id], lazy="select", viewonly=True
    )
