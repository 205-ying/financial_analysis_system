...
class User(Base, IDMixin, TimestampMixin):
    ...
    email: Mapped[str] = mapped_column(
        String(100), nullable=False, index=True, comment="邮箱"
    )
    ...
    full_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="真实姓名"
    )
    ...
    store_permissions: Mapped[list[UserStorePermission]] = relationship(
        "UserStorePermission", back_populates="user", cascade="all, delete-orphan"
    )
...
class Role(Base, IDMixin, TimestampMixin):
    ...
    code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="角色编码"
    )
...
class Permission(Base, IDMixin, TimestampMixin):
    ...
    # 基本信息
    code: Mapped[str] = mapped_column(
        String(100), nullable=False, index=True, comment="权限编码"
    )

    name: Mapped[str] = mapped_column(String(100), nullable=False, comment="权限名称")

    resource: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="资源标识"
    )
