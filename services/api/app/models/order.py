...
class OrderHeader(Base, IDMixin, TimestampMixin):
    ...
    order_no: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="订单号"
    )
    ...
    order_time: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, index=True, comment="下单时间"
    )
    ...
    table_no: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="桌号"
    )

    # 金额信息
    gross_amount: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), comment="商品总额"
    )

    discount_amount: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), comment="优惠金额"
    )

    service_charge: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), comment="服务费"
    )

    delivery_fee: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), comment="配送费"
    )

    net_amount: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=Decimal("0.00"), comment="实收金额"
    )

    # 支付信息
    payment_method: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="支付方式（cash/alipay/wechat/card/other）"
    )
    ...
    customer_name: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="客户姓名"
    )

    customer_phone: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="客户电话"
    )

    customer_address: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="客户地址"
    )

    # 其他信息
    remark: Mapped[str | None] = mapped_column(Text, nullable=True, comment="订单备注")
...
class OrderItem(Base, IDMixin, TimestampMixin):
    ...
    product_sku: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="产品SKU（快照）"
    )

    product_name: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="产品名称（快照）"
    )

    product_category: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="产品分类（快照）"
    )

    # 数量和价格
    quantity: Mapped[Decimal] = mapped_column(
        Numeric(10, 3), nullable=False, comment="数量"
    )

    unit: Mapped[str] = mapped_column(
        String(20), nullable=False, default="份", comment="单位"
    )
    ...
    line_amount: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, comment="小计金额"
    )
    ...
    remark: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="商品备注"
    )
