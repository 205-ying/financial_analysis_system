...
class ExpenseType(Base, IDMixin, TimestampMixin):
    ...
    type_code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="费用科目编码"
    )

    name: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="费用科目名称"
    )
    ...
    level: Mapped[int] = mapped_column(
        Integer, default=1, nullable=False, comment="科目层级"
    )
    ...
    description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="科目描述"
    )
    ...
    parent: Mapped[ExpenseType | None] = relationship(
        "ExpenseType", remote_side="ExpenseType.id", back_populates="children"
    )

    children: Mapped[list[ExpenseType]] = relationship(
        "ExpenseType", back_populates="parent", cascade="all, delete-orphan"
    )

    records: Mapped[list[ExpenseRecord]] = relationship(
        "ExpenseRecord", back_populates="expense_type"
    )
...
class ExpenseRecord(Base, IDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    amount: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, comment="费用金额"
    )
    ...
    invoice_no: Mapped[str | None] = mapped_column(
        String(50), nullable=True, index=True, comment="发票号码"
    )

    vendor: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="供应商"
    )

    # 支付信息
    payment_method: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="支付方式"
    )

    payment_account: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="支付账户"
    )
    ...
    submitted_at: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="提交日期"
    )
    ...
    expense_type: Mapped[ExpenseType] = relationship(
        "ExpenseType", back_populates="records"
    )
