...
class ReportQuery(BaseModel):
    ...
    group_by: Optional[str] = Field(
        None, description="分组字段（day/week/month/store）"
    )
...
class DailySummaryRow(BaseModel):
    ...
    operating_profit_rate: Optional[Decimal] = Field(
        None, description="营业利润率（%）"
    )
...
class MonthlySummaryRow(BaseModel):
    ...
    avg_daily_order_count: Decimal = Field(
        default=Decimal("0.00"), description="日均订单数"
    )
