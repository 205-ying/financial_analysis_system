...
# 上传文件配置
UPLOAD_BASE_DIR = settings.upload_path
...
class ImportService:
    ...
    async def create_job(
        ...
        job = DataImportJob(
            job_name=job_name
            or f"{target_type.value}_导入_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    async def _parse_file(job: DataImportJob) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ValidationException(f"文件解析失败: {str(err)}") from err

    @staticmethod
    async def _import_orders(
        db: AsyncSession, job: DataImportJob, rows: list[dict[str, Any]]
    ):
        """导入订单数据"""
        ...
        # 批量查询已存在的订单号
        order_nos = [
            get_field(row, "order_no", "") for row in rows if get_field(row, "order_no")
        ]
        existing_orders = await db.execute(
            select(OrderHeader.order_no).where(OrderHeader.order_no.in_(order_nos))
        )
        existing_order_nos = {result[0] for result in existing_orders.all()}
        ...
        for idx, row in enumerate(rows, start=1):
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"业务日期格式错误，应为 YYYY-MM-DD: {biz_date_str}"
                    ) from err

                try:
                    gross_amount = (
                        Decimal(str(gross_amount_str))
                        if gross_amount_str
                        else Decimal("0")
                    )
                    discount_amount = (
                        Decimal(str(discount_amount_str))
                        if discount_amount_str
                        else Decimal("0")
                    )
                    net_amount = (
                        Decimal(str(net_amount_str))
                        if net_amount_str
                        else gross_amount - discount_amount
                    )
                except (InvalidOperation, ValueError) as err:
                    raise ValueError(f"金额格式错误: {err}") from err
    ...
    @staticmethod
    async def _import_expense_records(
        db: AsyncSession, job: DataImportJob, rows: list[dict[str, Any]]
    ):
        """导入费用记录"""
        store_id = job.config.get("store_id")
        ...
        expense_types = {
            et.type_code: et.id for et in expense_types_result.scalars().all()
        }
        ...
        for idx, row in enumerate(rows, start=1):
            try:
                ...
                try:
                    ...
                    raise ValueError(f"业务日期格式错误: {biz_date_str}") from err

                try:
                    amount = Decimal(str(amount_str))
                except (InvalidOperation, ValueError) as err:
                    raise ValueError(f"金额格式错误: {amount_str}") from err
    ...
    @staticmethod
    async def _import_expense_types(
        db: AsyncSession, job: DataImportJob, rows: list[dict[str, Any]]
    ):
        """导入费用科目数据"""
        # TODO: 实现费用科目导入逻辑
