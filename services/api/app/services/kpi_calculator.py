...
class KpiCalculator:
    ...
    async def rebuild_daily_kpi(
        self, start_date: date, end_date: date, store_id: Optional[int] = None
    ) -> tuple[int, int, int]:
        """
        重建日指标数据（upsert 模式）
        ...
        """
        # 获取需要计算的门店列表
        store_query = select(Store.id).where(Store.is_active.is_(True))
        if store_id:
    ...
    async def _aggregate_orders(self, store_id: int, biz_date: date) -> dict:
        ...
        query = select(
            ...
            # 订单数（已完成）
            func.count(
                case((OrderHeader.status == "completed", OrderHeader.id), else_=None)
            ).label("order_count"),
            # 客流量（简化：使用订单数）
            func.count(
                case((OrderHeader.status == "completed", OrderHeader.id), else_=None)
            ...
            and_(OrderHeader.store_id == store_id, OrderHeader.biz_date == biz_date)
        )
    ...
    async def get_store_comparison(
        ...
        # 按门店分组
        query = query.group_by(KpiDailyStore.store_id, Store.code, Store.name).order_by(
            func.sum(KpiDailyStore.net_revenue).desc()
        )
        ...
        return [
            {
                ...
                "profit": float(row.profit or 0),
                "profit_rate": float(
                    (row.profit / row.revenue * 100) if row.revenue else 0
                ),
            }
            for row in rows
        ]
