...
class Settings(BaseSettings):
    ...
    # 运行时文件配置
    runtime_dir: str = Field(default="runtime", description="运行时文件根目录")
    upload_dir: str = Field(
        default="runtime/uploads/imports",
        description="导入文件存储目录",
    )

    # 日志配置
    log_level: str = Field(default="INFO", description="日志级别")
    log_file: Optional[str] = Field(
        default="runtime/logs/api/app.log",
        description="日志文件路径",
    )
    ...
    def runtime_path(self) -> Path:
        """运行时文件根目录。"""
        return self.resolve_project_path(self.runtime_dir)
    ...
    def log_file_path(self) -> Optional[Path]:
        """日志文件路径，兼容旧的 LOG_FILE=logs/app.log 写法。"""
        ...
        if (
            ...
            and path.parts
            and path.parts[0].lower() in {"log", "logs"}
        ):
            relative_log_path = (
                Path(*path.parts[1:]) if len(path.parts) > 1 else Path("app.log")
            )
            return self.runtime_path / "logs" / "api" / relative_log_path
