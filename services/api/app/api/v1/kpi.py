...
from app.services.data_scope_service import assert_store_access, filter_stores_by_access
...
async def get_kpi_summary(
    ...
    store_count_result = await db.execute(
        select(func.count(func.distinct(KpiDailyStore.store_id))).where(
            and_(*conditions)
        )
    )
...
async def get_kpi_trend(
    ...
    expense_query = select(
        ExpenseRecord.biz_date, func.sum(ExpenseRecord.amount).label("cost")
    )
...
async def get_store_ranking(
    ...
    sort_by: str = Query(
        "profit", description="排序字段：revenue/profit/profit_margin"
    ),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    # 查询费用数据（按门店）
    expense_query = select(
        ExpenseRecord.store_id, func.sum(ExpenseRecord.amount).label("cost")
    )
...
async def rebuild_kpi(
    ...
    if data.start_date > data.end_date:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="开始日期不能大于结束日期"
        )
    ...
    affected_dates, affected_stores, total_records = await calculator.rebuild_daily_kpi(
        start_date=data.start_date, end_date=data.end_date, store_id=data.store_id
    )
