...
from app.services.order_service import get_order_export_rows, get_order_list
...
@router.get("/export", summary="导出订单", description="导出订单列表为Excel文件")
async def export_orders(
    ...
    for idx, row in enumerate(rows, 1):
        ...
        ws.cell(
            row=idx + 1,
            column=6,
            value=row.OrderHeader.order_time.strftime("%Y-%m-%d %H:%M:%S")
...
@router.post(
    "", response_model=Response[dict], summary="创建订单", description="创建新的订单"
)
