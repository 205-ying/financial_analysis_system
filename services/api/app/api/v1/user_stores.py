...
async def get_user_stores(
    ...
    query = (
        ...
        .join(Store, UserStorePermission.store_id == Store.id, isouter=True)
        .where(UserStorePermission.user_id == user_id)
        .order_by(UserStorePermission.created_at.desc())
    )
...
async def assign_user_stores(
    ...
    for store_id in data.store_ids:
        permission = UserStorePermission(user_id=data.user_id, store_id=store_id)
        db.add(permission)
